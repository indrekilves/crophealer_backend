...
public class UserRestService extends GenericRestService
{
    ...
    {
        if ( ur == null )
            throw new BadRequestException( "User resource is missing" );

        RequestError re = Users.validateUserResource( ur );
        if ( re != null )
            throw new ConflictException( re.getValue() );
    ...
    public ResponseEntity < DiagnosedProblemResourceList > getDiagnosedProblemsForUserByLanguage( Long id,
            Languages language )
    {
        ...
        if ( id == null )
            throw new BadRequestException( "ID is missng" );
        if ( language == null )
            throw new BadRequestException( "Language is missng" );
    ...
    /*
     * 
     * private List<Message> getSentMessages(Users user, Integer offset, Integer
     * limit) { int firstResult = offset == null ? 0 : offset; int maxResults =
     * limit == null ? Integer.MAX_VALUE : limit; TypedQuery<Message> tq =
     * Message
     * .findMessagesBySender(user).setFirstResult(firstResult).setMaxResults
     * (maxResults); if (tq != null && tq.getResultList().size() > 0){ return
     * tq.getResultList(); } return null; }
     * 
     * 
     * private List<Message> getReceivedMessages(Users user, Integer offset,
     * Integer limit) { int firstResult = offset == null ? 0 : offset; int
     * maxResults = limit == null ? Integer.MAX_VALUE : limit;
     * TypedQuery<Message> tq =
     * Message.findMessagesByReceiver(user).setFirstResult
     * (firstResult).setMaxResults(maxResults); if (tq != null &&
     * tq.getResultList().size() > 0){ return tq.getResultList(); } return null;
     * }
     */
    ...
    public ResponseEntity < UserResourceList > getUsersByRole( String role )
    {
        ...
        if ( role == null )
            throw new BadRequestException( "Role is missng" );
        ...
        if ( auth == null )
            throw new ResourceNotFoundException( "Authority not found for role: " + role );
    ...
    public ResponseEntity < String > addAdvisorsToUser( Long userId, Long advisorId )
    {
        ...
        if ( advisorId == null )
            throw new BadRequestException( "Advisor ID is missing" );
        ...
        if ( advisor == null )
            throw new ResourceNotFoundException( "Advisor not found by ID: " + advisorId );
    ...
    public ResponseEntity < UserAdvisorResourceList > getUserAdvisorsForUserAndAdvisor( Long userId, Long advisorId )
    {
        ...
        if ( tq != null && tq.getResultList().isEmpty() )
            throw new ResourceNotFoundException( "User Advisors not found for user: " + userId + " advisor: "
                    + advisorId );
    ...
    public ResponseEntity < UserResourceList > getClientsForUser( Long id, String status )
    {
        ...
        if ( tq != null && tq.getResultList().isEmpty() )
            throw new ResourceNotFoundException( "User Advisors not found for user: " + id + " by status: " + status );
    ...
    public ResponseEntity < FieldResourceList > getFieldsForUser( Long id )
    {
        ...
        if ( fieldsSet.isEmpty() )
            throw new ResourceNotFoundException( "Fields not found for user: " + id );
    ...
    public ResponseEntity < CompanyResourceList > getCompaniesForUser( Long id )
    {
        ...
        if ( company == null )
            throw new ResourceNotFoundException( "Companies not found for user: " + id );
    ...
    public ResponseEntity < UserRoleResourceList > getRolesForUser( Long id )
    {
        ...
        if ( roles.isEmpty() )
            throw new ResourceNotFoundException( "Roles not found for user: " + id );
